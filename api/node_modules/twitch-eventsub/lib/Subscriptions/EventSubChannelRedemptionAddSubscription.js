"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.EventSubChannelRedemptionAddSubscription = void 0;
var tslib_1 = require("tslib");
var twitch_common_1 = require("twitch-common");
var EventSubChannelRedemptionAddEvent_1 = require("../Events/EventSubChannelRedemptionAddEvent");
var EventSubSubscription_1 = require("./EventSubSubscription");
/**
 * @private
 */
var EventSubChannelRedemptionAddSubscription = /** @class */ (function (_super) {
    tslib_1.__extends(EventSubChannelRedemptionAddSubscription, _super);
    function EventSubChannelRedemptionAddSubscription(handler, client, _userId, _rewardId) {
        var _this = _super.call(this, handler, client) || this;
        _this._userId = _userId;
        _this._rewardId = _rewardId;
        return _this;
    }
    Object.defineProperty(EventSubChannelRedemptionAddSubscription.prototype, "id", {
        get: function () {
            return "channel.channel_points_custom_reward_redemption.add." + this._userId;
        },
        enumerable: false,
        configurable: true
    });
    EventSubChannelRedemptionAddSubscription.prototype.transformData = function (data) {
        return new EventSubChannelRedemptionAddEvent_1.EventSubChannelRedemptionAddEvent(data, this._client._apiClient);
    };
    EventSubChannelRedemptionAddSubscription.prototype._subscribe = function () {
        return tslib_1.__awaiter(this, void 0, void 0, function () {
            var _a, _b, _c, _d, _e, _f;
            return tslib_1.__generator(this, function (_g) {
                switch (_g.label) {
                    case 0:
                        if (!this._rewardId) return [3 /*break*/, 2];
                        _b = (_a = this._client._apiClient.helix.eventSub).subscribeToChannelRedemptionAddEventsForReward;
                        _c = [this._userId,
                            this._rewardId];
                        return [4 /*yield*/, this._getTransportOptions()];
                    case 1: return [2 /*return*/, _b.apply(_a, _c.concat([_g.sent()]))];
                    case 2:
                        _e = (_d = this._client._apiClient.helix.eventSub).subscribeToChannelRedemptionAddEvents;
                        _f = [this._userId];
                        return [4 /*yield*/, this._getTransportOptions()];
                    case 3: return [2 /*return*/, _e.apply(_d, _f.concat([_g.sent()]))];
                }
            });
        });
    };
    EventSubChannelRedemptionAddSubscription = tslib_1.__decorate([
        twitch_common_1.rtfm('twitch-eventsub', 'EventSubSubscription')
    ], EventSubChannelRedemptionAddSubscription);
    return EventSubChannelRedemptionAddSubscription;
}(EventSubSubscription_1.EventSubSubscription));
exports.EventSubChannelRedemptionAddSubscription = EventSubChannelRedemptionAddSubscription;
